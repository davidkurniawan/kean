...
class Profile extends CI_Controller {
	...
	public function index($value='')
	{
		...
		$viewData['transUser'] = $this->GlobalModel->queryManual('SELECT * FROM transaction_order tor JOIN status_transaksi sta ON tor.transaction_status=sta.id_status_transaksi WHERE tor.id_user_customer="'.$this->session->userdata('idAdmin').'" AND (tor.transaction_status="1" OR tor.transaction_status="6") ');
		$viewData['transUserSettle'] = $this->GlobalModel->queryManual('SELECT * FROM transaction_order tor JOIN status_transaksi sta ON tor.transaction_status=sta.id_status_transaksi WHERE tor.id_user_customer="'.$this->session->userdata('idAdmin').'"  ');
		$viewData['profile']	= $this->GlobalModel->getDataRow('user_customer',array('id_user_customer'=>$this->session->userdata('idAdmin')));
		$viewData['referral']	= $this->GlobalModel->getData('referral_code_voucher',array('id_user_customer'=>$this->session->userdata('idAdmin')));
		$viewData['historyClaim'] = $this->GlobalModel->queryManual('SELECT ssitus.title_ekspedisi_status, uscr.nama,rpn.nomer_resi,uscr.alamat,uscr.kecamatan,uscr.provinsi,uscr.kode_pos,uscr.kota,rpn.signature,rpn.created_date,rpn.status_redeem_poin,ppn.produk_poin_title,ppn.poin,ppn.image FROM redeem_poin rpn JOIN user_customer uscr ON rpn.id_user_customer=uscr.id_user_customer JOIN produk_poin ppn ON rpn.id_produk_poin=ppn.id_produk_poin JOIN ekspedisi_status ssitus ON rpn.status_redeem_poin=ssitus.id_ekspedisi_status WHERE rpn.id_user_customer="'.$this->session->userdata('idAdmin').'" ');
		$viewData['kupon'] = $this->GlobalModel->getData('promo_product',array('id_user_customer'=>$this->session->userdata('idAdmin')));
		$viewData['proses'] = $this->GlobalModel->queryManual('SELECT * FROM transaction_order tor JOIN status_transaksi sta ON tor.transaction_status=sta.id_status_transaksi JOIN ekspedisi_status epstus ON tor.ekspedisi_status=epstus.id_ekspedisi_status WHERE tor.id_user_customer="'.$this->session->userdata('idAdmin').'" AND tor.ekspedisi_status="2" ');
		$viewData['pengiriman'] = $this->GlobalModel->queryManual('SELECT * FROM transaction_order tor JOIN status_transaksi sta ON tor.transaction_status=sta.id_status_transaksi JOIN ekspedisi_status epstus ON tor.ekspedisi_status=epstus.id_ekspedisi_status WHERE tor.id_user_customer="'.$this->session->userdata('idAdmin').'" AND tor.ekspedisi_status="3" ');
		$viewData['arrived'] = $this->GlobalModel->queryManual('SELECT * FROM transaction_order tor JOIN status_transaksi sta ON tor.transaction_status=sta.id_status_transaksi JOIN ekspedisi_status epstus ON tor.ekspedisi_status=epstus.id_ekspedisi_status WHERE tor.id_user_customer="'.$this->session->userdata('idAdmin').'" AND tor.ekspedisi_status="4" ');
